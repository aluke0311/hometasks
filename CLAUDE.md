...
- **`dealMax` governs the dealer, not a cap** — `startExtraCycle` runs a second load by hand, and `startCycle` puts it straight into `state.hand` so it's on TODAY's list rather than waiting for tomorrow's deal.
- A cycle untouched for `CYCLE_STALE_DAYS` (3) shows "still going?" with **Drop it** and **Finished it**.
- **A stage can borrow another cycle's wait, via `waitsOn: { cycle, fromStage }`** (2026-08-22). The fountain's `reassemble` stage is cleaned IN the dishwasher, so its 129 minutes only mean anything once a dishwasher cycle is actually on `unload` (the stage that means the machine is running) — before this, starting the fountain counted down on its own, so a fountain started 11 hours before the dishwasher ever ran still read "ready." `cycleWaitState(c)` resolves this: `{ blocked, readyAt }`, where `blocked` means there is nothing to show at all (no host cycle running, or the host was dropped mid-wait — never a stale time). `cycleIsReady` and `cycleReadyLabel` both go through it; `cycleReadyLabel` prints `'waiting on the <host label>'` while blocked. Never auto-starts the host cycle — she may be loading it over an hour, and the fountain waits, it does not drive.
- **A cycle's controls live on the card face, not the swipe rail** (2026-08-22, audit C-5). Her standing rule from here on: **nothing new goes on the swipe rail — More / Pin / Remove only, on every card.** A running cycle renders a `.cyc-face-actions` row under the status line instead: Back (only once `stage > 0`) / Drop it / Finished it. Not shown in the Sprint's compact rows — Drop it stays reachable from the task page there. **The row lives inside `.task-card` but OUTSIDE `.task-swipe`'s positioning context** — the swipe rail and the row it slides over are wrapped in their own `.task-row` (`position:relative; overflow:hidden`), because the rail's old `top:0;bottom:0` used to stretch against the whole card, and once the face-actions row made cycling cards taller than plain ones, the rail showed through beside the face buttons instead of staying clipped behind `.task-main`.
- **Advancing a step (`advanceCycle`) has an Undo**, via the toast's own action button, calling `cycleBack`. `cycleBack` in turn **deletes the cycle rather than setting `stage: 0`** when undoing the very first step — a stage-0 cycle is exactly the state that used to strand a load in the Sprint's Laundry block while its own card had already gone back to "Into the washer." (`getSprintBlocks`'s `SPRINT_LOAD_IDS` treats "no cycle" and "stage 0" as the same "still has to be started" case, belt-and-braces, for a stage-0 cycle arriving some other way.)
- **The task page's primary button on a cycle owner advances one step**, labeled with the step's own action (never "Mark done", never the current phase — the phase is what the status line already says). A separate row, **"Mark whole cycle done,"** finishes the whole load via `finishCycle`.
- **`cycleHasHadItsTurn(defKey)`** is the one definition of "this cycle's slot is filled for today" — dealt (running up to `dealMax`), pinned, in progress, completed today by an owner, or already dealt into `state.hand` even if not yet tapped. `dealHand`'s own cycle-slot loop and `getOverflowTasks()` both call it, so "+ Give me more" can't suggest a second load of laundry when the dealer has already decided today's is spoken for. **"Start another" is unaffected** — it stays possible as an explicit choice on the task page.
- **Per-task machine timings** (2026-08-22): `stageReadyIn(defKey, stageIdx, taskId)` checks `state.cycleTimings[taskId + '.' + stageKey]` first, then `state.cycleTimings[defKey + '.' + stageKey]`, then the built-in `readyIn`. She runs a different washer programme for whites than for towels, so "one wash time" was never really a property of the *cycle* — it's a property of the *load*. Every override saved before this split still works, as the fallback. The timings editor shows the per-task value when editing an existing task (blank = falls back, with the shared default as the input's placeholder); with no task id yet (still creating) it edits the shared, definition-level default, same as before. **Labeled by what the wait measures** (the phase, e.g. "In the washer"), not by the stage it precedes (e.g. "Into the dryer") — getting that backwards is how a dryer time ends up typed into the washer field.
...
- `cycleTimings` — `{'<def|taskId>.<stageKey>': minutes}` her overrides for machine time. Keyed by definition (the old, still-working shared default) **or by task id** (2026-08-22, per-load — e.g. `'lroom_whites.dry'` overrides just that load's wash time); `stageReadyIn` checks task id first, then definition, then the built-in `readyIn`
...
- **Nothing new goes on the swipe rail** (2026-08-22, audit C-5). It's revealed only by a sideways drag, so anything living only there is invisible until you already know it exists. The rail stays More / Pin / Remove, everywhere; a control common enough to need its own face (a running cycle's Back / Drop it / Finished it) goes on the card itself.
