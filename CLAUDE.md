...
- **A cycle's Back / Drop it / Finished it live on the task page only, never on the card.** A 2026-08-22 pass first put them on the card face (`.cyc-face-actions`), reasoning they were common enough to earn a visible spot; she pushed back the same day — she did not want them sitting on every running cycle's card, every day. Reverted: the rail stays plain More / Pin for a cycling card (no Remove — dropping a running load isn't "remove"), and Back / Drop it / Finished it are reachable only via the task page (More, the ⋯ button, or long-press). **The card's only nudge is the "still going?" badge** once `cycleIsStale` (untouched `CYCLE_STALE_DAYS`, currently 3) — that badge is text in the meta line, not a button; acting on it still means opening the task page. Her standing rule stays: **nothing new goes on the swipe rail — More / Pin / Remove only, on every card.**
